...
func TestGetNextVersion(t *testing.T) {
	...
	var tests = []getNextVersionTest{
		{"php:7.99-fpm", false, nil, false, "php:8.0-fpm"},
		{"php:7.2-fpm", false, &major, false, "php:7.3-fpm"},
		{"php:7.2-fpm", false, &minor, false, "php:7.2-fpm"},
		{"php:7.2.0-fpm", false, nil, true, "php:8.0.1-fpm"},
		{"php:7.2.0-fpm", false, &patch, true, "php:7.2.0-fpm"},
		{"php:7.2-fpm", false, &patch, false, "php:7.2-fpm"},
		{"php:7.2-fpm", false, nil, false, "php:7.3-fpm"},
		{"php:7.3-fpm", false, nil, false, "php:7.4-fpm"},
		{"php:7.3-xfpm", false, nil, false, "php:7.3-xfpm"},
		{"deadbeef:1.0", true, nil, false, "deadbeef:1.0"},
		{"golang:1.12.0", false, &minor, true, "golang:1.12.17"},
		{"golang:1.12.99", false, &major, true, "golang:1.15.7"},
		{"golang:1.12.99", false, &minor, false, "golang:1.12.99"},
		{"index.docker.io/library/golang:1.12.6", false, &minor, true, "index.docker.io/library/golang:1.12.17"},
		{"golang:latest", false, nil, false, "golang:latest"},
		{"golang:least", false, nil, false, "golang:least"},
		{"php:28.1-fpm", false, nil, false, "php:28.1-fpm"},
	}
	for _, test := range tests {
		i, _ := name.ParseReference(test.input)
		input, _ := i.(name.Tag)
		output, err := GetNextVersion(input, test.limit, test.latest)
		if err == nil {
